...
(() => {
  ...
  appHeader=function(title='PartFit Ghana',sub='Right Part. Right Car.'){
    ...
    return `<header class="top v3Top"><button class="brandMark" data-page="home" aria-label="PartFit Ghana — home"><img class="brandLogo" src="./logo.svg" alt="" width="56" height="32"></button><div class="title">${P.esc(title)}<small>${P.esc(sub)}</small></div><div class="grow"></div>${topNav}<button class="topAction" data-help>WA</button><button class="cartBtn" data-page="order">🛒 <b>${cartCount()}</b></button>${accountTop()}</header>`;
  };
